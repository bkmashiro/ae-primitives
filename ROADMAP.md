...
The original spatial-factory roadmap is complete. PneumaticCraft now has tiered compressed-air cells for normal AE2 drives, bounded pressure import/export ports and an ME Pneumatic Assembly Chamber that consumes the matching air domain directly from its grid. The selectable physical-quantity display, capability inspector, bounded crafting forecast, deterministic virtual commissioning, Flight Recorder and Crafting Autopsy are complete. Factory diagnostics retain only bounded owner-local state transitions and build short causal explanations on request in the existing Process Analyzer. The current pointer is Phase 6 of the [production intelligence roadmap](docs/production-intelligence-roadmap.md): the short-lived, player-requested ME Network Lens.
