...
Add the smallest persisted extension-owned stage contract required by the Runic Altar. A packaged idle, empty Runic Altar Interface must reserve its concrete ingredients and livingrock reagent transactionally into one explicit adjacent real altar, retain native recipe identity and mana progress across factory reload, wake from real resource changes, and recover only its own native output. One real altar may serve only one lane at a time; blocked output or component removal must not lose or duplicate reserved work. Keep Botania classes and opaque stage interpretation in the extension, and do not turn the factory into a generic polling coordinator.
