...
Add the Petal Apothecary as the next extension-owned native stage. A packaged idle, empty interface must reserve its concrete petals and seed reagent transactionally into one explicit adjacent, water-filled real apothecary, persist its opaque stage identity across factory reload, wake from water and topology changes, and recover only its own native output. One apothecary may serve only one lane at a time; blocked output or component removal must not lose or duplicate reserved work. Keep Botania classes and native stage interpretation in the extension rather than adding Botania semantics to Core.
