...
The original spatial-factory roadmap is complete. PneumaticCraft now has tiered compressed-air cells for normal AE2 drives, bounded pressure import/export ports and an ME Pneumatic Assembly Chamber that consumes the matching air domain directly from its grid. The selectable physical-quantity display, capability inspector, bounded crafting forecast and deterministic virtual commissioning are complete. Commissioning uses copied machine configuration, a bounded synthetic input ledger and non-collectible resource descriptions, and rejects probabilistic or world-native machines before planning. The current pointer is Phase 5 of the [production intelligence roadmap](docs/production-intelligence-roadmap.md): Flight Recorder and Crafting Autopsy built together over bounded causal events.
