...
/** Runs one packaged Runic Altar Interface against one exclusive, explicit real altar. */
public final class RunicAltarVirtualLaneExecutor implements VirtualMachineLaneExecutor {
    public static final RunicAltarVirtualLaneExecutor INSTANCE = new RunicAltarVirtualLaneExecutor();

    @Override public boolean supports(MachineSpaceEnvelope envelope) {
        return envelope.blockId().equals(BuiltInRegistries.BLOCK.getKey(BotaniaContent.RUNIC_ALTAR_INTERFACE.get()));
    }

    @Override public LanePlan prepare(LaneContext context) {
        RunicAltarInterfaceBlockEntity port = findPort(context);
        ItemStack expected;
    ...
    private static RunicAltarInterfaceBlockEntity findPort(LaneContext context) {
        for (Direction direction : Direction.values()) {
            if (context.level().getBlockEntity(context.factoryPos().relative(direction))
                    instanceof RunicAltarInterfaceBlockEntity port) return port;
        }
        return null;
    }

    private RunicAltarVirtualLaneExecutor() {}
}
