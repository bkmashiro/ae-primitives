...
def main() -> None:
    ...
    for model_id in MODEL_IDS:
        ...
        models.append(model)
        images.update(used)
    for model_id in ("me_press", "me_crusher", "me_catalyst_chamber", "me_basin_processor", "me_filling_station"):
...
body.export{background:#10161c;overflow:hidden}body.export header,body.export .gui{display:none}body.export main,body.export .grid,body.export .card{margin:0;padding:0;max-width:none;width:100vw;height:100vh}body.export .grid{gap:0}body.export .card{border:0;border-radius:0;box-shadow:none}body.export .card h2{display:none}body.export .card canvas{display:block;width:100vw;height:100vh}
</style></head>
...
function render(canvas,model){const ctx=canvas.getContext('2d');const dpr=devicePixelRatio||1,w=exportMode?innerWidth:(canvas.clientWidth||380),h=exportMode?innerHeight:w*.82;canvas.width=w*dpr;canvas.height=h*dpr;ctx.scale(dpr,dpr);ctx.clearRect(0,0,w,h);const yaw=+(model.view?.yaw??document.querySelector('#yaw').value)*Math.PI/180,pitch=+(model.view?.pitch??document.querySelector('#pitch').value)*Math.PI/180,scale=Math.min(w,h)*(exportMode?.52:.60)/Math.max(...model.span),cx=w*.5,cy=h*(exportMode?.46:.5);let faces=[];
