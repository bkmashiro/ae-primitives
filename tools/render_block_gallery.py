...
function render(canvas,model){const ctx=canvas.getContext('2d');const dpr=devicePixelRatio||1,w=canvas.clientWidth||380,h=exportMode?w:w*.82;canvas.width=w*dpr;canvas.height=h*dpr;ctx.scale(dpr,dpr);ctx.clearRect(0,0,w,h);const yaw=+document.querySelector('#yaw').value*Math.PI/180,pitch=+document.querySelector('#pitch').value*Math.PI/180,scale=Math.min(w,h)*.044,cx=w*.5,cy=h*.52;let faces=[];
